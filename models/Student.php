...
class Student
{
    ...
    // Saare students fetch karo (basic info)
    ...
    // Student ki file names fetch karo
    ...
    // Email exists check (students + users)
    ...
    // Student update karo (with/without password)
    ...
    // Students with teacher name (join)
    ...
    // Search student by student_id string
    ...
    // Assign teacher to student (transaction)
    public function assignTeacherToStudent($student_id, $teacher_id)
    {
        ...
        try {
            ...
            $stmt->bind_param("i", $student_id);
            if (!$stmt->execute()) {
                throw new Exception('Failed to remove student from previous batches.');
            }
            $stmt->close();
    ...
    // New student create karo
    public function createStudent($data, $student_pic_name, $cnic_pic_name, $password)
    {
    ...
    // Teacher ke assigned students fetch karo (without contact/email)
...
// Private helper: course end date calculate karo
...
// Teacher ke unassigned students with details fetch karo
...
// Student search karo teacher ke under with course progress
public function searchStudentWithProgress($student_id_str, $teacher_id)
{
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // Current month and days elapsed
